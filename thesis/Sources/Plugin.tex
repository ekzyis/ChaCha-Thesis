...
At the bottom, if we are still in the ChaCha hash function loop, you can see the visualization of the quarterrounds (Figure \ref{fig:chachahash.mid.qr}). If we are finished with the loop, you can see the original state at the left, the result of adding the original state with the state after all rounds in the middle, and at the right the result of reversing the byte order of each state entry of the state in the middle (Figure \ref{fig:chachahash.end}). This is the final state and thus one block inside the keystream with which the input message is later XOR'ed . \\
...
For example, if he wants to go to the second quarterround of the fourth round, he can enter "4" into the text input which is labeled with "Round" and press Enter or use the arrow buttons to go to the fourth round. This will take him to the start of the second round. He can then proceed to enter "2" into the "Quarterround" text input. Since this (as with the other input fields) wil take him to the start of the quarterround, he can use the buttons to the right of the quarterround input to jump to the end of specific quarterrounds of the current round. For example, if he now wants to go to the end of the second quarterround, he can click on "End" of the second button of the buttons which are labeled with "Diagonalround". Figure \ref{fig:chachahash.cr} and Figure \ref{fig:chachahash.dr} show how the page looks like at the end of all quarterrounds. As you can see, the corresponding state entry is highlighted with a light blue background.\\
The button labels to the right of quarterround navigation show the state indices that will get updated during their quarterround in parentheses and will be update according to the round. This means on uneven rounds, the label will switch to "Columnround" and on even rounds, it will show "Diagonalround".
...
\begin{figure}
\caption{End of quarterround execution of all column rounds}
...
\caption{End of quarterround execution of all diagonal rounds}
