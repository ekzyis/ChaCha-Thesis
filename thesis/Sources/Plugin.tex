...
In the first subsection, the key features of the plugin are described including pictures to give a rough overview what a user can expect from the actual plugin implementation.
...
As mentioned in \ref{sec:goals}, if the user wants to see the diffusion property of the cipher, he can enter secondary values on a dedicated page inside the plugin. This is shown in Figure \ref{diffusion}. Using the input values of the user, the cipher is then executed and the ChaCha hash function is visualized. Following the terms Prof. Bernstein used in his specification paper, the 512-bit state is visualized as a 4$\times$4 matrix. This makes it more clear to the user why some quarterrounds are called column rounds while others are called diagonal rounds. In Figure \ref{chachahash}, you can see the execution of a quarterround; visualized as a diagram very similar to the one which can be found on Wikipedia. The current state is shown in the top left corner.
...
\begin{subfigure}[t]{.5\textwidth}
  ...
  \caption{Hash function visualization}
  \label{diffusion.inactive}
\end{subfigure}%
\begin{subfigure}[t]{.5\textwidth}
  \centering
  \includegraphics[width=0.99\textwidth]{figures/diffusion-active}
  \caption{Hash function visualization with active diffusion}
