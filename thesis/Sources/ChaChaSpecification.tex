...
The \textit{quarter-round} function, the \textit{little-endian} function, a hash function which utilizes the two other mentioned functions, how the 512-bit state is setup and finally, how the encryption/decryption process works as a whole, bringing all the individual pieces together. 
...
After a set amount of rounds (8, 12, or 20), the input vector is added to the vector on which the quarter-round function was run. Then the byte order of each matrix entry is reversed using the little-endian function.
